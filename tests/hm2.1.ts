...
describe('[HM] Поиск кота и добавление лайков', async () => {
  ...
  it('2. Получение и сохранение лайков', async () => {
    ...
    await allure.step(`Получение лайков у кота: ${expectedCat.name}`,
      async () => {
        console.warn('Тест 2 ☑', 'Выполняем извлечение количества лайков');
        current_likes = expectedCat.likes;
        allure.testAttachment(
          'Информация о количестве лайков',
          JSON.stringify(`likes: ${expectedCat.likes}`, null, 2),
          'application/json',
        );
        //Честно, не совсем понимаю что проверять в обычном парсе данных
        assert.ok(current_likes === expectedCat.likes, `Значения не совпадают ${current_likes} / ${expectedCat.likes}`,
        );
      });
  ...
  it('3. Добавление лайков коту ', async () => {
    ...
    await allure.step(`Добавление лайков коту: ${expectedCat.id}`,
      async () => {
  ...
  it('4. Проверка количества лайков у кота ', async () => {
    await allure.step(` Проверка количества лайков у кота: ${expectedCat.id}`,
      async () => {
